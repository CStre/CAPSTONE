...
| File                   | Purpose                                                                              |
| ---------------------- | ------------------------------------------------------------------------------------ |
| `HomePage.tsx`         | Page root — orchestrates auth state, canvas, loader, and card selection              |
...
| `AuthGreetingCard.tsx` | Auto-advancing greeting card for signed-in users; dismissable via "Play with Phil"   |
| `IntroCard.css`        | All card-specific styles (glass, progress timeline, explore button) — shared by both |
| `useIntroSlides.ts`    | State machine for the 6-slide visitor sequence (entering → visible → exiting phases) |
